...
- Prioritise tasks already marked `in_progress` before starting new ready
  tasks.
...
- The main Ralph agent remains the only agent allowed to edit files, update
  Beads, add comments, and run git commands.
- After implementation and validation, run a blocking two-pass review gate
  before the final commit.
- The review gate must use exactly four read-only reviewer sub-agents driven
  by the reviewer prompt sections in this file:
  - implementation reviewer
  - quality reviewer
  - testing reviewer
  - simplicity reviewer
- Reviewers are advisory only. They must not edit files, update Beads,
  add comments, or run git commands.
- The main Ralph agent must provide reviewer context. Do not rely on reviewers
...
- Verify every reported finding against the actual code and nearby context
  before acting on it.
...
- Second pass must ignore non-blocking, stylistic, documentation-only, and
  minor simplification concerns unless they create a real correctness,
  security, or maintainability risk for this task.
- Log when second-pass sign-off review starts.
- Log whether second-pass sign-off passed or failed.
- If second pass reports blocking issues, fix them, rerun validation, and
  rerun the appropriate review pass.
- Allow at most 2 review/fix cycles after the initial first review pass.
- A review/fix cycle means: fix confirmed blocking findings, rerun validation,
  then rerun the appropriate review pass.
- Log when each review/fix cycle completes.
- Only close a task and make a non-WIP completion commit when blocking
...
- Wrapper adds nothing: method just calls another method with the same
  signature.
- Factory for single implementation: factory pattern when only one concrete
  type exists.
- Interface on producer side: interface defined where implemented, not where
  consumed.
- Layer cake anti-pattern: handler -> service -> repository when each just
  passes through.
- DTO or mapper overkill: multiple types representing the same data with
  conversion functions.

#### Premature Generalization

- Generic solution for specific problem: event bus for one event type.
- Config objects for 2-3 options: options pattern when direct parameters
  suffice.
- Plugin architecture for fixed functionality: extension points nothing
  extends.
- Overloaded struct: one type handling all variations with many optional
  fields.
...
- Silent fallbacks hiding problems: catching errors and falling back instead
  of failing fast.
