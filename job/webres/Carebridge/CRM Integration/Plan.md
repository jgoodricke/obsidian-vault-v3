# CRM Integration Initial Production Release estimate
...
**Proposed approval labour envelope: 180 person-days.**
...
| ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------: |
| Integration Core foundation               | Tenant and Connection model, durable records, PostgreSQL work ledger, leases, checkpoints, scheduling, scoped service authentication, and minimum audit.                   |          26 |
| Carebridge machine boundary               | Read projections, Integration Principal, policy scope, idempotent domain commands, optimistic concurrency, native side effects, and link display projection.               |          22 |
| Provider link-or-create workflow          | Carebridge modal, server-to-Core calls, search and exact-ID UX, stale-preview control, link-existing, create-new confirmation, and result display.                         |          16 |
| Resident Select adapter and outbound Flow | Basic authentication, pagination, lookups, search, exact reads, Prospect creation, demographic comparison and update, retry classification, and reconciliation.            |          20 |
| Inbound workflow Flow                     | Evidence snapshots, precedence, progression lanes, Proposed Actions, approval and automatic paths, terminal controls, revalidation, and Carebridge transition calls.       |          22 |
| Administration and operations             | Local role access, credentials, mappings, health, approvals, conflicts, safe retry, exceptional linking, redacted history, and bounded notifications.                      |          20 |
| AWS delivery                              | Staging and production infrastructure, GitHub Actions, Fargate, RDS, secrets, private networking, production HTTPS, telemetry, backup controls, and deployment procedures. |          18 |
| Cross-cutting verification and hardening  | Integration and browser tests, concurrency tests, idempotency, recovery, credential rotation, privacy and log review, reconciliation, and release stabilization.           |          24 |
| Test Organisation and first Connection    | Synthetic validation, pilot mappings, first live onboarding, controlled writes, Provider acceptance, and production support.                                               |          12 |
| **engineering labour**                    |                                                                                                                                                                            |     **180** |
